...
################################################################################
# Intune Pre-Installation Script for Mac Dev Setup
...
if [ "$AVAILABLE_SPACE" -lt 30 ]; then
    log "WARNING: Less than 30GB disk space available. Installation may fail."
    log "Available: ${AVAILABLE_SPACE}GB, Recommended: 30GB"
...
if ping -c 1 github.com &> /dev/null; then
    ...
    log "WARNING: Cannot reach github.com. Installation requires internet access."
    # Don't exit, Homebrew installation will fail if no internet
fi

# Check if Xcode Command Line Tools are installed (optional but helpful)
if xcode-select -p &> /dev/null; then
    log "✓ Xcode Command Line Tools detected"
else
    log "INFO: Xcode Command Line Tools not found. Homebrew will install them automatically."
fi
...
# Create user notification (will show in Company Portal)
log "========================================="
log "Pre-Installation Checks Complete"
log "========================================="
log "System is ready for Mac Dev Setup installation"
